...
- **14 predictors** (climate, topography,  human impact, etc)
...
.center[## Comparing the models with spatial cross-validation]
...
comparison.df <- data.frame(
  ...
  intrinsic.rmse = c(
    ...
    model.spatial$performance$rmse
  ),
  extrinsic.rmse = c(
    median(
...
kableExtra::kbl(
  comparison.df, 
  format = 'html',
  col.names = c("Model", "spatial predictors", "Moran's I of residuals", "RMSE oob", "RMSE spatial cv")
  )
```
...
importance.df <- rbind(
  ...
  dplyr::mutate(
    model = "Spatial",
    variable = ifelse(variable == "spatial_predictors", "spatial_predictors_mem", variable)
    )
) %>% 
...
ggplot(data = plot.df) + 
  ...
  geom_path(size = 1) + 
  facet_wrap("predictor.name", scales = "free_x", ncol = 2) + 
  theme_bw(base_size = 20) +
    labs(
...
- **Spatial predictors** reduce SAC in model residuals (**GOOD**)
- **Spatial predictors** hinder model transferability (**KINDA BAD**)
- **Eigenvectors**: Outcome similar to base model, plus information about the importance of spatial processes (**GOOD**)
- Computational demands limit sample size
...
.medium[
Incorporating **eigenvectors of the distance matrix** into spatial analyses might improve understanding on underlying spatial processes not represented by the covariates.
]
